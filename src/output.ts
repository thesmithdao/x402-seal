...
export function printEvidence(evidence: Evidence, path: string, json: boolean): void {
  ...
  const settlement = evidence.settlement?.success ? "PASS" : evidence.verdict === "UNKNOWN" ? "UNKNOWN" : "FAIL";
  ...
  process.stdout.write([
    ...
    row("SETTLEMENT", settlement, evidence.settlement?.success ? "terms matched" : "not confirmed"),
